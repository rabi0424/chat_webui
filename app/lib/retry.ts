...
/**
 * 空いた枠が少ないうちは担当を起こさず、まとまってから起こすか。
 *
 * 依頼1本あたりの実行体の時間は「生成にかかる時間 ÷ 担当1つが同時に
 * 持っている本数」で決まる。1本ずつ起こすと、その1本が生成時間まるごとを
 * 負う——**依頼ごとに実行体を分けるのと同じ**で、禁じているはずの形に
 * 戻ってしまう。
 *
 * 実測（並列8・24秒の生成・14本）では、最初の一束のあと成功が1件返る
 * たびに1本だけ起こしていて、実効の同時数が 2.5本まで落ちていた。枠を
 * 埋め続けるより、空きが溜まってからまとめて起こすほうが、同じ枠で
 * 何倍も走れる（速さは落ちる。利用者はそれを承知で本数を採る）。
 *
 * ただし**何も走っていないときは待たない**。待つ相手がいないので、
 * 待てば実行が止まるだけ。これで進まなくなることはない。
 */
export function shouldLaunchWave(input: {
  /** いま起こせる本数（枠・上限・柵の残りのうち最小）。 */
  room: number;
  /** 走っている本数。 */
  running: number;
  /** 開けておく枠の数。 */
  ...
  if (input.room <= 0) return false;
  if (input.running === 0) return true;
  // 枠の半分が空くまで待つ。1本あたりの費用は最良の2倍までに収まる
  return input.room * 2 >= input.slots;
}
